...
$sql1 = "SELECT * FROM worker ORDER BY id DESC";
            $result1 = $conn->query($sql1);
            $num_rows = mysqli_num_rows($result1);
?>
<title><?php echo $site_name . ' - Add Workers' ?></title>
</head>
<body class="bg-gray-900">
     <div class="container mx-auto py-10 sm:py-10 px-4">
         <h3 class="text-2xl font-semibold  text-white mb-5">Cloudflare Workers Status
         <span style="border: 2px solid #1f2937; padding: 0px 20px; background: linear-gradient(268deg, #3f83f8, #06b6d4); text-align: center; display: inline-block; border-radius: 8px; font-size: smaller;">
             <?php 
             echo $num_rows;
             ?>
             </span>
    ...
    <table class="w-full text-sm text-left text-gray-500 text-gray-400">
        ...
        <tbody>
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                <th scope="row" class="py-4 px-6 font-medium  whitespace-nowrap text-white">'. $row["wk"] . '</th>
